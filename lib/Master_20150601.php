	...
	public function getUnitAddress($lookup, $unitname,$type)
	{
	  ...
	  $qry = $qry.$condition;
	   ...
	   if($row = $this->oDb->get_row($qry))
	   {
				...
				{$addressFormat.='PHONE :'.$row->phone.' <br>';}if( $row->fax !="")
...
public function addInventory($aRequest,$files)
		...
		$result = $this->oDb->get_results($qry_po);
			 foreach($result as $row)
			{
				...
				if($ordered_qty >= $receivedqty )
				{
					...
					$update_status = '13';
					$qry_update_status = "UPDATE po_item SET status='".$update_status."',qty_received='".$receivedqty."',balanced_qty='".$quantity."' WHERE id_po_item=".$poitemId;
					$this->oDb->query($qry_update_status);
					$astatusGrn['status'] =$update_status;
					$astatusGrn['bal_qty'] =$quantity;
				}
	...
	public function addInventoryItem($aRequest)
	{
		...
		if(empty($aid_po_item)){$aid_po_item = array();}	
	...
	$qry ="
			...
			if($id_po!='0') { $qtys = $this->checkQuanityByPO($items[8],$items[3],$id_po);}
		 ...
		 if( count($qtys)> 0 )
					{
						$qty_status = $qtys['status'];
						$bal_qty = $qtys['bal_qty'];
						if($qty_status =='14' || $bal_qty <=0)
							{
	...
	public function updateInventoryItem($aRequest)
	{
		...
		if(empty($aid_po_item)){$aid_po_item = array();}
		if(empty($aItemGroup1)){$aItemGroup1 = array();}
		if(empty($aItemGroup2)){$aItemGroup2 = array();}
		if(empty($aitem)){$aitem = array();}
		if(empty($quanity)){$quanity = array();}
		if(empty($uom)){$uom = array();}
		if(empty($id_manufacturer)){$id_manufacturer = array();}
		if(empty($unitprice)){$unitprice = array();}
		if(empty($item_requireddate)){$item_requireddate = array();}
		if(empty($item_remark)){$item_remark = array();}
...
public function getInventoryInfo($lookup, $type)
 ...
 // Stock list
	...
	public function addStockItem($aRequest)
	{
		...
		$created_by        = $_SESSION['sesCustomerInfo']['user_id'];
		$aItemGroup1Info = $this->getItemGroup1Info($aRequest['fGroup1'][0]);
		$assetType = $aItemGroup1Info['id_asset_type'];
		$aRequest['fAssetType'][0] = $assetType;
		...
		if($Item[0]>0)
		{
		$qry = "INSERT INTO asset_item(id_asset_item, id_asset_category, id_asset_type, id_itemgroup1, id_itemgroup2, quantity, id_uom, id_vendor, id_manufacturer, id_unit,machine_no,warranty_start_date,warranty_end_date,asset_no,ref_asset_no,id_grn, id_inventory_item, id_po, asset_name, asset_amount, depressation_percent, remark, date_of_install,machine_date,machine_life, machine_price,created_by,created_on,modified_by,modified_on,status) VALUES (NULL,'', '$Item[17]','$Item[1]','$Item[2]','$Item[3]','$Item[4]','$Item[5]','$Item[6]','$aItemgroup[1]','".strtoupper($Item[9])."','$Warranty_start','$Warranty_end','".$Asset_numbers."','$Item[7]','','','','$Item[0]','$Item[16]','$Item[12]','','".date('Y-m-d',strtotime($Item[13]))."','".date('Y-m-d',strtotime($Item[10]))."','$Item[11]','','$created_by',now(),'','','1')";
			//exit();
		if($this->oDb->query($qry))	{
...
VALUES (NULL ,   '$file_name',  '$file_type', '1');";
  ...
  public function getItemGroup1List()
  {
		...
		if($result = $this->oDb->get_results($qry))
		{
			foreach($result as $row)
			{
				...
				$aItemGroup['lookup']          = strtoupper($row->lookup);
                                $aItemGroup['id_asset_type']   = $row->id_asset_type;
	...
	public function addItemGroup1($aRequest)
	{
		...
		$id_asset_type = $aRequest['fAssetType'];
		$status          = $aRequest['fStatus'];
		$qry = "INSERT INTO itemgroup1 (id_itemgroup1, itemgroup1_name, lookup, id_asset_type, status) VALUES (null, '$itemgroup1_name','$lookup', '$id_asset_type', '$status')";
	...
	{
	   $qry = "SELECT id_itemgroup1, itemgroup1_name, lookup, id_asset_type, status FROM itemgroup1 WHERE ";
	   if($type == 'lookup') {
	   	 $condition = " lookup = '$lookup'";
	   }
	   else {
	...
	public function updateItemGroup1($aRequest, $action) //action -> update, delete
	{
		...
		{
          $qry = "UPDATE itemgroup1 SET itemgroup1_name = '$itemGroupName', lookup = '$itemGroupName', id_asset_type = '$id_asset_type', status = '$status' WHERE id_itemgroup1 = ".$id_itemgroup1;
			   $this->oDb->query($qry);
				if( mysql_affected_rows() >= 0)
				{
				$valid = '1';
...
public function getPOTaxInfo($lookup, $type)  //lookup may be 'id' or 'Lookup text'
	...
	public function getAssetTypeListHTML()
	{
		$qry = "SELECT * FROM asset_type WHERE status != 2";
		$html .= '<select name="fAssetType" class="contact-input" id="contact-country" tabindex="1004">';
		$html .= '<option value="0">--Select Asset Type--</option>';
		if($result = $this->oDb->get_results($qry))
		{
			foreach($result as $row)
			{
				$html .= '<option value="'.$row->id_asset_type.'">'.strtoupper($row->asset_type_name).'</option>';
			}
		}
		$html .= '</select>';
		return $html;
...
public function getDeliveryItemList()
  	 ...
  	 public function getDeliveryList($type=null)
  {
		...
		elseif($type == 'Receipt')
		{
			$qry = "SELECT * FROM asset_delivery WHERE status = 16";
		}
		else
		{
  ...
  public function getDeliveryPrint($assetDeliveryId,$type='')
  {
	  $aPrint = array();
	...
	if($type == 'print')
	 {
		 ...
		 $aDeliveryItem = $this->getDeliveryItemInfoListprint($assetDeliveryId,'delivery');
	 }
	 else
	 {
  ...
  public function getDeliveryItemInfo($lookup, $type)  //lookup may be 'id' or 'Lookup text'
	...
	/*new function*/
	 public function getDeliveryItemInfoListprint($lookup, $type)  //lookup may be 'id' or 'Lookup text'
	{
...
/*new code below*/	
	public function addFuelToken($aRequest)
	{
		...
		if($this->oDb->query($trip_qry))
	    {
		 ...
		 $qry = "INSERT INTO fuel_token(id_fuel_token, token_no, token_date, id_fuel_type, qty, oil_qty, id_asset_item, id_trip, id_vendor, id_employee, remarks, created_by, created_on, modified_by, modified_on, status) VALUES (NULL,'$token_number','$token_date', '$fuel_type','$qty','$oil_qty','$id_asset_item','$lastInsertId','$id_vendor','$id_employee','$remarks','$created_by ',now(),'','','1')";
		 if($this->oDb->query($qry))
	   {
	     //exit();
	     return true;
		 ...
		 $qry = "SELECT * FROM asset_item WHERE status != 2";
		   if($type == 'id') {
			 ...
			 $condition = " AND id_grn = '$lookup'";
			   ...
			   $condition = " AND id_asset_type = '$lookup'";
		   }
		   else {
			 $condition = " AND asset_no = ".$lookup;
		   }
...
public function ItemListDisplay($itemgrouptwoid,$fgroup2)
  {
		$qry = "SELECT item.*, itemgroup2.id_itemgroup1 FROM item INNER JOIN itemgroup2 ON item.id_itemgroup2 = itemgroup2.id_itemgroup2 WHERE item.id_itemgroup2=".$itemgrouptwoid." AND itemgroup2.id_itemgroup1=".$fgroup2;
		$aItemList = array();
		if($result = $this->oDb->get_results($qry))
		{
			foreach($result as $row)
			...
			$aGroup1 = $this->getItemGroup1Info($aGroup2['id_itemgroup1'],'id');
			$aItem['itemgroup1_name']       = $aGroup1['itemgroup1_name'];
			$aItem['id_itemgroup1']       = $aGroup1['id_itemgroup1'];
			$aItem['itemgroup2_name']       = $aGroup2['itemgroup2_name'];
				$aItem['status']    = $row->status;
				$aItemList[]        = $aItem;
			}
...
public function getGroup2ItemDisp($lookup, $type,$lookup1 = '',$lookup2='')
	{
		$qry = "SELECT Distinct itemgroup_item_map.id_itemgroup2, itemgroup2.id_itemgroup1,itemgroup2.itemgroup2_name FROM itemgroup_item_map INNER JOIN itemgroup2 ON itemgroup_item_map.id_itemgroup2 = itemgroup2.id_itemgroup2 WHERE itemgroup_item_map.id_itemgroup1=".$lookup;
		$aItemGroupList = array();
	   if($result = $this->oDb->get_results($qry))
	   {
		 foreach($result as $row)
			{
				...
				$aItemGroup['id_itemgroup1']   = $row->id_itemgroup1;
		        $aItemGroup['id_itemgroup2']   = $row->id_itemgroup2;
				$aItemGroup['itemgroup2_name'] = $row->itemgroup2_name;
				$aItemGroupList[] = 	$aItemGroup ; 
			}
	...
	public function getItemDisp($lookup, $type,$lookup1 = '',$lookup2='')
	{
		 $qry = "SELECT Distinct itemgroup_item_map.id_item,item.id_item,item.id_itemgroup2,item.item_name FROM itemgroup_item_map INNER JOIN item ON itemgroup_item_map.id_item = item.id_item WHERE itemgroup_item_map.id_itemgroup2=".$lookup;
		 ...
		 foreach($result as $row)
			{
				...
				$aItemGroup['id_itemgroup2']   = $row->id_itemgroup2;
				$aItemGroup['id_item']   = $row->id_item;
				$aItemGroup['item_name']  = $row->item_name;
				$aItemGroupList[] = 	$aItemGroup ; 
			}

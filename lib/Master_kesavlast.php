...
/////////////////////////////////////////////////////////////////////////////////////////	
	...
	public function updateState($aRequest, $action)
	{
	    if($action == 'update')
		{
		  ...
		  $lookup     = strtoupper($aRequest['fLookup']);
		  $status     = $aRequest['fStatus'];
		  $qry = "UPDATE state SET state_name = '$state_name', id_country = '$id_country', lookup = '$lookup', status = '$status' WHERE id_state = ".$id_state;
		}
		else if($action == 'delete')
		{
		  $id_state  = $aRequest['fStateId'];
		  $qry = "UPDATE state SET status =2 WHERE id_state =".$id_state;
		}
		$valid = false;
		if($this->oDb->query($qry)) {
		 $valid = true;
	...
	{
	    $qry = "SELECT * FROM city WHERE status!=2";
		$aCityList = array();
		if($result = $this->oDb->get_results($qry))
		{
			foreach($result as $row)
	...
	public function updateCity($aRequest, $action)
	{
		...
		{
		  $id_city  = $aRequest['fCityId'];
		  $qry = "UPDATE city SET status = 2 WHERE id_city =".$id_city;
		}
		$valid = false;
		if($this->oDb->query($qry)) {
		 $valid = true;
	...
	{
	    $qry = "SELECT * FROM division WHERE status !=2";
		$aDivisionList = array();
		if($result = $this->oDb->get_results($qry))
		{
			foreach($result as $row)
...
$bill_tot_amount =$aRequest['fBillTotalAmount']; 
 ...
 $aInsertvalues = array_map(null,$aRequest['fAssetItemId'],$aRequest['fBillAmount']);
	...
	{
	    $qry = "SELECT * FROM taxform WHERE status != 2";
		$aTaxFormList = array();
		if($result = $this->oDb->get_results($qry))
		{
			foreach($result as $row)
	...
	public function updateUnitAddressId($reff_id,$lastInsertId)
   {
	...
	{
		 $qry = "SELECT * FROM vendor_contact_map WHERE id_vendor=".$id;
...
public function addPurchaseRequest($aRequest)
	   ...
	   if($action == 'update')
...

		{
			...
			$this->addHistoryTransLog($aRequest,'','','PR',$Trans,$PRNumber,$id_pr,$status,$qry,'Purchase Request Approved','');
			  ...
			  $qry = "UPDATE purchase_request SET id_department='".$id_department."',id_unit='".$id_unit."',id_employee='".$id_employee."',terms_and_conditions='".$terms_condtions."',require_date='".$requiredate."',remarks='".$remarks."',modified_by='$created_by',modified_on=now() WHERE id_pr = ".$id_pr;
 ...
 $qry ="INSERT INTO po_item(id_po_item,id_po, id_pr, id_vendor, po_number, id_asset, vendor_part_no, id_itemgroup1, id_itemgroup2, id_item, po_item_name, unit_cost, qty, qty_received,balanced_qty,id_uom,id_manufacturer, due_date, remark, modified_by, modified_date, approved_by, approved_date, status) VALUES (NULL, '$lastInsertId','','$id_vendor', '', '', '','$items[0]','$items[1]','$items[2]','','$items[6]','$items[3]','','$items[3]','$items[4]','$items[5]','".date('Y-m-d H:i:s',strtotime($items[7]))."','$item_remark', '', '', '$created_by', now(), '1')";
		...
		{	
			...
			INSERT INTO po_item(id_po_item,id_po, id_pr, id_vendor, po_number, id_asset, vendor_part_no, id_itemgroup1, id_itemgroup2, id_item, po_item_name, unit_cost, qty, qty_received,balanced_qty,id_uom,id_manufacturer, due_date, remark, modified_by, modified_date, approved_by, approved_date, status) VALUES (NULL, '$lastInsertId','','$id_vendor', '', '', '','$items[0]','$items[1]','$items[2]','','$items[6]','$items[3]','','$items[3]','$items[4]','$items[5]','".date('Y-m-d H:i:s',strtotime($items[7]))."','$item_remark', '', '', '$created_by', now(), '1')";
	...
	$id_vendor     = $aRequest['fvendorId'];
			...
			$qry = "UPDATE po_item SET id_po='".$id_po."',id_pr='".$id_pr."',id_vendor='".$id_vendor."',id_itemgroup1='".$items[0]."',id_itemgroup2='".$items[1]."',id_item='".$items[2]."',unit_cost='".$items[6]."',qty='".$items[3]."',balanced_qty='".$items[3]."',id_uom='".$items[4]."',id_manufacturer='".$items[5]."',due_date='".date('Y-m-d H:i:s',strtotime($items[7]))."',remark='$item_remark',modified_by='$created_by',modified_date=now() WHERE id_po_item=".$items[8];
...

{
...

	{
		 ...
		 $qry = "SELECT * FROM inventory WHERE status!=2 ";

		 if($type == 'grn') {

			$condition = "and id_inventory IN( $strings_ids) ORDER BY id_inventory DESC";

		   }

		   else if($type == 'asset') {

			$condition = "and id_inventory IN( $strings_ids1) ORDER BY id_inventory DESC";
 ...
 public function addAssetItem($aRequest,$files)
...

	{
		...
		$terms = $aRequest['fTerms'];				
		  ...
		  $qry = "INSERT INTO  contract (id_contract ,contract_type ,contract_title,id_vendor,contract_start_date ,contract_end_date ,contract_reference_number ,remark,contract_date ,renewal_date,no_items, terms_and_conditions)VALUES (NULL ,  '$contract_type','$contract_title','$id_vendor', '$contract_start_date',  '$contract_end_date',  '$contract_reference_number','$remark', '$contract_date',  '$renewal_contract_date','$no_items','$terms');";
...
VALUES (NULL ,   '$file_name',  '$file_type', '1');";
...

  {
...

	{
...

		{
		  ...
		  $qry = "UPDATE itemgroup1 SET status = 2 WHERE id_itemgroup1 =".$id_itemgroup1;
...

  {
...

	{
		...
		$qry = "INSERT INTO itemgroup2 (id_itemgroup2, itemgroup2_name, lookup,id_itemgroup1,status) VALUES (null, '$itemgroup2_name','$lookup', '$id_group1','$status')";
...

	{
...

		{
		  ...
		  $qry = "UPDATE itemgroup2 SET status = 2 WHERE id_itemgroup2 = ".$id_itemgroup2;
...

  {
		...
		$qry = "SELECT * FROM item WHERE status != 2";
...

  {
		...
		$qry = "SELECT * FROM manufacturer WHERE status != 2";
...
public function getDeliveryItemList()
  ...
  public function getDeliveryItemInfo($lookup, $type)  //lookup may be 'id' or 'Lookup text'
		...
		$aContractList = array();
...

		   {
...

			{
				   ...
				   $aAssetContractDocument = $this->getContractDocumentInfoList($aContract['id_contract'],'id');
	...
	$qry = "SELECT * FROM terms_conditions WHERE status != 2";
...
FROM
    ...
    INNER JOIN item 
        ...
        ON (fuel.id_fuel_type = item.id_item) WHERE  fuel.status !=2 ";
...
FROM
    ...
    INNER JOIN item 
        ...
        ON (fuel.id_fuel_type = item.id_item) WHERE  asset_item.status !=2 ";
...
/// Insurance
	...
	/*  echo '<pre>';
	  ...
	  $insurance_date = date('Y-m-d H:i:s',strtotime($aRequest['fInsuranceDate']));
		...
		INSERT INTO insurance(id_insurance, id_vendor, policy_amount, premium_amount, insurance_policy_name, ins_start_date, ins_end_date, remark, renewal_date,created_by, created_on, modified_by, modified_on, status) VALUES (NULL,'$id_vendor','$insurance_ordervalue','$insurance_value',' $insurance_title','$insurance_start_date','$insurance_end_date','$remark','$renewal_insurance_date',' $created_by ',now(),'','','1')
...
VALUES (NULL ,   '$file_name',  '$file_type', '1');";
		 ...
		 $qry = "SELECT * FROM insurance WHERE id_insurance = ".$row->id_insurance;
...

		   {
...

			{
					...
					$aInsurance['remarks']   = $row->remarks;
...

	{
		 ...
		 $qry = "SELECT * FROM asset_insurance WHERE status!=2 and id_asset = ".$lookup;

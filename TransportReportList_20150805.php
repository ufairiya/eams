<?php
  ...
  if(isset($aRequest['report']))
  {
	  if($aRequest['fItemName'] == '' || $aRequest['fItemName'] == '0')
  	  {
	  	$aFuelTokenDetails = $oTransport->getFuelTokenInfoAll($aRequest);
  	  }
	  else
	  {
	  $aFuelTokenDetails = $oTransport->getFuelTokenInfo($aRequest);
	  }
  ...
  //echo '<pre>';
  //print_r($aRequest);
  //print_r($aFuelDetails['assetinfo']);
  //print_r($aFuelTokenDetails);
  //echo '</pre>';
  //exit();
  ...
  if(isset($_REQUEST['send']))
	{
	 if(empty($sessReportFuelList))
		{
		   $sessReportFuelList = array();
		}
		$sessReportFuelList = $aRequest;
		$oSession->setSession('sess_ReportFuellist',$sessReportFuelList);
	}
	$rowsPerPage = 20;
  	$pageNum = 1; 
	?>
             ...
             <div class="row-fluid profile">
					<div class="tabbable tabbable-custom">
							<div id="tab_1_5" class="tab-pane">
                                ...
                                <?php if(isset($aRequest['report'])) { ?>
                                <a href="TransportReport.php?report=<?php echo $aRequest['report']; ?>">BACK TO FORM</a>
                                <?php } else { ?>
                                <a href="TransportReport.php">BACK TO FORM</a>
                                <?php } ?>
                                <div><a href="TransportReportListPDF.php">Print PDF</a></div>
                                </div>
...
?> 
<tr>
<td><?php echo $fueldetailsresults['machine_no']?></td>
<td><?php echo $fueldetailsresults['asset_no']?></td>
<td>
<?php if($fueldetailsresults['asset_image']!='') 
{
?>
       <span class="photo"> <img src="uploads/assetimage/<?php echo $fueldetailsresults['asset_image'];?>" alt="" height="50" width="50"/></span>
      <?php } else {?>
            <span class="photo">   <img src="assets/img/noimage.gif" alt="" height="50" width="50" /></span>
     <?php } ?>

</td>
<td><?php echo date('d-m-Y',strtotime($fueldetailsresults['warranty_start_date']));?></td>
<td><?php echo date('d-m-Y',strtotime($fueldetailsresults['warranty_end_date']));?></td>
</tr>
<?php 
} 
} }
...
<td>
<?php if($fueldetailsresults['assetinfo']['asset_image']!='') 
{
?>
...
<?php if(isset($aRequest['report'])) { ?>
    ...
    <th>CMR</th>
	 ...
	 <?php 
	foreach($aFuelTokenDetails as $aFuel) 
	{
	 ?>
    <tr>
    <td><?php echo date('d-m-Y',strtotime($aFuel['token_date']));?></td>
    <td><?php echo $aFuel['token_no'];?></td>
    <td><?php echo $_aFuelType[$aFuel['id_fuel_type']];?></td>
    <td><?php echo $aFuel['qty'];?></td>
    <td><?php echo $aFuel['oil_qty'];?></td>

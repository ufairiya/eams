<?php
  ...
  $aRequest = $_REQUEST;
   if(empty($aRequest['fQuantity']))
  {
	  $aRequest['fQuantity'] = 1;
  }
...
<body class="fixed-top">
                              ...
                              <div class="portlet-body form">
								 ...
								 <?php		                                     
                                    if(!empty($errMsg))
										...
										?> 
                                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" class="form-horizontal" id="form_pos" method="post" enctype="multipart/form-data">
                               ...
                               <div class="controls">
                                       ...
                                       <select class="span8 "   tabindex="4"  data-placeholder="Choose a Unit Id" name="fUnitId" id="fUnitId" onChange="getDivision(this.value);">
											  <option value=""></option>
											<?php
 ...
 <input type="hidden" class="testprice" name="fTaxTotals[]" id="taxprices<?php echo $i;?>" value="0"/>                              
  ...
  <input type="hidden" class="testprice" name="fTaxTotals[]" id="taxprices<?php echo $i;?>" value="0"/>                            
                   ...
                   <td><input type="text" class="m-wrap xsmall" name="fQuantity[]" value="<?php echo $purchaseitem['balanced_qty'];?>"  <?php if($purchaseitem['qty_received']!='0'){ echo 'disabled="disabled"'; }?>  onKeyUp="qtytotalpricecalc(this.id);  checkQty(this.id,this.value,'<?php echo $aRequest['id'];?>','<?php echo $purchaseitem['id_pr_item'];?>');"id="qty<?php echo $i;?>" style="margin-bottom: 5px;"/>
                                          <select class="small m-wrap chosen" data-placeholder="Choose a UOM" name="fUOMId[]">
                                               <option value=""></option>
											 <?php
											  $aUOMList= $oMaster->getUomList();
 ...
 <input type="hidden" class="testprice" name="fTaxTotals[]" id="taxprices<?php echo $i;?>" value="0"/>                       
                           ...
                           <?php 
								...
								<tr>
                                  <td>
                                          ...
                                          <select class="m-wrap margin group1" name="fGroup1[]" id="fGroupItem1" onChange="getGroup2ItemListings(this.value,this.id);">
                                            <option value="0" selected="selected" >Choose the ItemGroup 1 </option>
											 <?php
											  $aItemGroup1List = $oMaster->getItemGroup1List();
											  foreach($aItemGroup1List as $aItemGroup1)
                                          ...
                                          <select class="m-wrap group2" name="fGroup2[]" id="Group2ItemList1" onChange="getItemListings(this.value,this.id);">
                                               <option value="0" selected="selected" >Choose the ItemGroup 2 </option>
											 <?php
											  $aItemGroup2List = $oMaster->getItemGroup2List();
											  foreach($aItemGroup2List as $aItemGroup2)
                   ...
                   <td><input type="text" class="m-wrap xsmall" name="fQuantity[]" value="1" onKeyUp="qtytotalpricecalc(this.id);"id="qty<?php echo $j;?>" style="margin-bottom: 5px;"/>
...
if($aRequest['action'] == 'edit' && $aRequest['submits']=='approval') {
 ...
 <script type="text/javascript">
		 ...
		 function getGroup2ItemListings(value,id)
		 {
			var dataStr = 'action=getGroup2ItemDisp&Group1Id='+value;
			$.ajax({
			   type: 'POST',
			   url: 'ajax/ajax.php',
			   data: dataStr,
		 ...
		 function getItemListing(value,id,itemid)
		 {
		  ...
		  function getItemListings(value,id)
		 {
			
			var dataStr = 'action=getItemDisp&Group2Id='+value;
			 $.ajax({
			   type: 'POST',
			   url: 'ajax/ajax.php',
			   data: dataStr,
	  ...
	  function checkQty(id,value,pr,pritemid)
	  {
			 ...
			 if($results.status == 0)
				{
				alert("Please Enter the Quantity less than or equal to Quantity : "+$results.avail_qty);
